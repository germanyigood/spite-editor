...
import { CircuitBoard, Scissors, Lightbulb, Film, MonitorUp, Crop, Cable, Grid3X3, Palette, Scaling, Gauge, CircleDashed, Layers, Sparkles, LucideIcon, ImagePlus, PaintBucket, Brush, Trash2 } from 'lucide-react';
...
const ITEMS: MenuItem[] = [
    ...
    { type: 'pixelize', label: 'Smart Pixelize', icon: Grid3X3, colorClass: 'text-cyan-400 hover:bg-cyan-500/20' },
    ...
    { type: 'frame_normalize', label: 'Frame Size', icon: Scaling, colorClass: 'text-cyan-400 hover:bg-cyan-500/20' },
...
const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, onClose, onAddNode, isEdgeAction, allowedTypes, onDeleteConnection }) => {
    ...
    const groups = [
        ['generate', 'paint', 'pixelize'],
        ['chroma', 'fill_color', 'composite', 'color_correct', 'grid'],
        ['outline', 'drop_shadow'],
        ['normal_map', 'seamless'],
        ['timeline', 'frame_normalize', 'crop', 'resize', 'optimize']
    ];
    ...
    return (
        ...
        >
            <div className="px-3 py-1.5 text-[10px] text-gray-500 font-bold uppercase tracking-widest border-b border-white/5 mb-1 flex items-center justify-between">
                <span>{isEdgeAction ? 'Connection' : 'Add Node'}</span>
                {isEdgeAction && <Cable size={10} className="text-yellow-500" />}
            </div>

            {isEdgeAction && onDeleteConnection && (
                <>
                    <button 
                        onClick={() => { onDeleteConnection(); onClose(); }}
                        className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:text-red-400 hover:bg-red-500/10 flex items-center gap-2"
                    >
                        <Trash2 size={14} className="shrink-0"/> Delete Wire
                    </button>
                    <div className="my-1 h-px bg-white/5 mx-3" />
                    <div className="px-3 py-1 text-[9px] text-gray-600 font-bold uppercase tracking-widest">Insert Node</div>
                </>
            )}
