...
export const Simulator = {
    ...
    async waitFor(selector: string, timeout = 10000): Promise<HTMLElement> {
        ...
        throw new Error(`Visible element "${selector}" not found after ${timeout}ms`);
    },
    ...
    async dblClick(selectorOrEl: string | HTMLElement) {
        ...
        const createClick = (detail: number, type: string) => 
            new MouseEvent(type, { bubbles: true, cancelable: true, view: window, detail, clientX: x, clientY: y, buttons: 1 });
    ...
    async drag(selectorOrEl: string | HTMLElement, deltaX: number, deltaY: number) {
        const el = typeof selectorOrEl === 'string' ? await this.waitFor(selectorOrEl) : selectorOrEl;
        const rect = el.getBoundingClientRect();
        ...
        const eventInit = { clientX: startX, clientY: startY, bubbles: true, buttons: 1, view: window };
        el.dispatchEvent(new MouseEvent('mousedown', eventInit));
        const steps = 12; 
        for (let i = 1; i <= steps; i++) {
            ...
            const moveEvent = new MouseEvent('mousemove', { ...eventInit, clientX: curX, clientY: curY });
            window.dispatchEvent(moveEvent);
            document.documentElement.dispatchEvent(moveEvent);
            await new Promise(r => setTimeout(r, 40));
        }
        window.__E2E_CURSOR__?.up();
        const upEvent = new MouseEvent('mouseup', { ...eventInit, clientX: startX + deltaX, clientY: startY + deltaY });
        window.dispatchEvent(upEvent);
        document.documentElement.dispatchEvent(upEvent);
        await new Promise(r => setTimeout(r, getDelay() + 500));
    },

    async injectTestImage(name: string = "TEST_ASSET.png", bgColor: string = "#FFFFFF", textColor: string = "#000000", w = 512, h = 512) {
