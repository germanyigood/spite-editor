...
// Export standard project file (.sforge)
export const createProjectBundle = async (animations: AnimationEntry[]): Promise<Blob> => {
  ...
  for (const anim of animations) {
    ...
    const { imageSrc, ...animRest } = anim as any; 
...
// Export separate PNGs + TS import file (.zip)
export const createTsCodeZip = async (animations: AnimationEntry[]): Promise<Blob> => {
    const zip = new JSZip();
    ...
    for (const anim of animations) {
      ...
      const safeName = anim.name.replace(/[^a-zA-Z0-9]/g, '_');
      const fileName = `${safeName}.png`;
  
      // Add image to zip
      zip.file(fileName, blob);
  
      // Add typescript code logic
      imports.push(`import ${safeName}Img from './${fileName}';`);
      dataEntries.push(`  "${anim.name}": {
    ...
    const tsContent = `// Generated by SpriteForge
// Contains imports for standard bundlers (Vite/Webpack/etc)
...
export type SpriteKey = keyof typeof SpriteData;
`;
