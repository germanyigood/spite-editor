...
## Организация тестов (Один файл — один тест)
Вы **ДОЛЖНЫ** строго придерживаться правила "один файл — один тест" для модульных тестов и тестов процессоров. Нельзя смешивать тесты для разных нод, разных компонентов, разного функционала в одном большом `spec` файле (например, `processor.spec.ts`). 
Если вы тестируете `FrameSkipNode` функционал, создайте отдельный файл `frameSkip.spec.ts` с четкой изоляцией и подключением в `StartupTestRunner.tsx`.
